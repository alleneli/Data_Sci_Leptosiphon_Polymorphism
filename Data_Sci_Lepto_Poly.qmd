...
Finally, for our intercept, we will set it as normal(0,1). The intercept is the baseline log-odds of a flower being pink when distance to shore is zero. The 0 means we expect the baseline probability to be centered around 50/50 (since plogis(0) = 0.5).
The 1 means we allow moderate uncertainty around that, letting the baseline probability range roughly between ~18% and ~82% (plogis(-1) to plogis(1)). 
```{r}
# Define priors
priors <- c(
